...
for subdir, dirs, files in os.walk(x):
    for file in files:
        dh = DH.DataHandler(subdir + os.sep + file, None, conf.syn)
        dataset = file
        ...
        for train, test in kfold.split(keys):
            ...
            for epoch in test:
                ...
                adapt_worker(dh, X_seq, X_mat, y_seq, matM, matN, ir, svd, bpr, None, None, None, crnn_model_adapt, multi_model)

                eval_worker(dh, X_feat, X_seq, y_single, None, None, None, crnn_model_eval, multi_model)
                ...
                res_adapt_eval, count_adapt_eval = AnE.deep_adapt_and_evaluate_multi(dataset,np.array(dh.inv_trans[epoch]),
                                                                            'Multi', multi_model, X_seq,
                                                                            y_seq, y_single, res_adapt_eval,
                                                                            count_adapt_eval)                                                            
            i += 1
        #print(res_adapt_eval.loc[0])
        #print(res_adapt_eval)
        ts = time.time()
        st = datetime.datetime.fromtimestamp(ts).strftime('%d_%m_%Y_%H_%M')
