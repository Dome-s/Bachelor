...
def deep_adapt_and_evaluate(dataset,instance, _type, adaptor, is_adaptor_mat, evaluator, is_evaluator_mat, X_seq, X_mat, y,
    ...
    k_adapt = 0
    #print(y)
    #print(np.ceil(np.array(X_seq.reshape(yhat_full.shape))))
...
def deep_adapt_and_evaluate_multi(dataset,instance, _type, multi, X_seq, y, y_single, res_adapt_eval, count_adapt_eval):
...
def reg_adapt_bpr(instance, _type, X, X_seq, y, adaptor, res_adapt, count_adapt):
    ...
    res_row_adapt = np.concatenate((instance, _type, str(k_adapt),
                                    precision_recall_fscore_support(y,
                                                                    np.ceil(np.array(X_seq.reshape(yhat_full.shape))),
                                                                    average='binary')[:3],
                                    precision_recall_fscore_support(y, yhat_full, average='micro')[:3]),
                                   axis=None)
    res_adapt.loc[count_adapt] = res_row_adapt
    count_adapt += 1
    return res_adapt, count_adapt
