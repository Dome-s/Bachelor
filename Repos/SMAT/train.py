...
class Instructor:
    ...
    def _evaluate_acc_f1(self, data_loader):
        ...
        #torch.argmax(t_outputs_all, -1).cpu() seems to contain current results gets called during 
        # training too but can probably just be called with extra indicator to only save in from execution after training
        path = 'C:/Users/dominic/Desktop/Bachelor/ADnEV and SMAT PreProc/SMATFinalResults/'
        ...
        print(path+self.opt.dataset_file['test'].split('/')[-1].split('.')[0]+'.csv')
        with open(path+self.opt.dataset_file['test'].split('/')[-1].split('.')[0]+'.csv'  ,'w',newline='') as f:
            w = csv.writer(f)
            df = pandas.DataFrame(torch.argmax(t_outputs_all, -1).cpu())

            for index,row in df.iterrows():
                w.writerow([df.iat[index,0]])
    ...
    def run(self):
        ...
        self._reset_params()
        if(self.opt.train_new == 1):
            best_model_path = self._train(criterion, optimizer, train_data_loader, val_data_loader, self.opt.lrshrink, self.opt.minlr)
        else:
            #gather best variant of trained models and try to use them for current task (sometimes not possible due to different embedding dimensions as they are fitted to current task)
            
            models = list()
            for subdir, dirs, files in os.walk('C:/Users/dominic/Desktop/Bachelor/Repos/SMAT/state_dict'):
                for dir in dirs:
                    for subdir2, dirs2, files2 in os.walk('C:/Users/dominic/Desktop/Bachelor/Repos/SMAT/state_dict/'+dir):
                        for file in files2:
                            models.append(subdir2+"/"+file)
            bestModels = set()
            for model in models:
                biggest = model
                name = model.split("/")[-1].rsplit("_",1)[0]
                max = model.split("/")[-1].rsplit("_",1)[1]
                for modelJ in models:
                    if (modelJ.split("/")[-1].rsplit("_",1)[0] == name and modelJ.split("/")[-1].rsplit("_",1)[1] > max):
                        max = modelJ.split("/")[-1].rsplit("_",1)[1]
                        biggest = modelJ
                bestModels.add(biggest)
            count = 0
            exception = list()
            for i in bestModels:
                try:
                    count += 1
                    self.model.load_state_dict(torch.load(i))
                    break
                except Exception as e:
                    if(count < len(bestModels)):
                        exception.append(e)
                        pass
                    else:
                        print(exception)
...
def main():
    ...
    parser.add_argument('--train_new', default=1, type=int, help='set flag = 0 or 1 weather a new model should be trained or one is chosen from disc')
    ...
    'test':{
        'train': 'C:/Users/dominic/Desktop/Bachelor/ADnEV and SMAT PreProc/SMATPreProcessedData/SMATtrainingFilexlsx.xlsx',
        'val': './datasets/omap/test_val.xlsx',
           'test': './datasets/omap/test_test.xlsx'
    },
    'valentine':{
        'train': 'C:/Users/dominic/Desktop/Bachelor/ADnEV and SMAT PreProc/SMATPreProcessedData/SMATtrainingFilexlsx.xlsx',
        'val': 'C:/Users/dominic/Desktop/Bachelor/ADnEV and SMAT PreProc/SMATPreProcessedData/SMATvalFilexlsx.xlsx',
           'test': 'C:/Users/dominic/Desktop/Bachelor/ADnEV and SMAT PreProc/SMATPreProcessedData/SMATMatchingTasksNotInTraining/assays_both_0_1_ac2_ev_.xlsx'
    },
    'valentine783':{
        'train': 'C:/Users/dominic/Desktop/Bachelor/ADnEV and SMAT PreProc/SMATPreProcessedData/SMATtrainingFilexlsx.xlsx',
        'val': 'C:/Users/dominic/Desktop/Bachelor/ADnEV and SMAT PreProc/SMATPreProcessedData/SMATvalFilexlsx.xlsx',
           'test': 'C:/Users/dominic/Desktop/Bachelor/ADnEV and SMAT PreProc/SMATPreProcessedData/SMATMatchingTasksNotInTraining/assays_both_0_70_ac2_ev_.xlsx'
    },
    ...
    # logger.addHandler(logging.FileHandler(log_file))
    if(opt.train_new == 0):
        for subdir, dirs, files in os.walk('C:/Users/dominic/Desktop/Bachelor/ADnEV and SMAT PreProc/SMATPreProcessedData/SMATMatchingTasksNotInTraining'):
                for file in files:
                    if('xlsx' in file and 'assays' not in file and 'miller2' not in file and 'musicians' not in file and 'both' not in file):
                        opt.dataset_file['test'] = subdir+"/"+file
                        opt.dataset = file
                        try:
                            opt.train_new = 0
                            ins = Instructor(opt)
